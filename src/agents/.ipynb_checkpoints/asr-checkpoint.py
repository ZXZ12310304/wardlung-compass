...
def _normalize_audio_to_wav16k_mono(audio_path: str, force_resample_wav: bool = True) -> Tuple[str, bool]:
    """
    把任意音频转为 16kHz mono wav。
    返回 (wav_path, is_temp)，is_temp=True 表示用完删除。
    """
    if not audio_path:
...
def _from_pretrained_compat(fn, *, token: Optional[str], trust_remote_code: bool, **kwargs):
    """
    transformers 不同版本 token 参数名不同：token vs use_auth_token
    """
    try:
        if token:
...
class MedASRTranscriber:
    """
    ✅ 只使用 google/medasr
    ✅ 不使用 transformers ASR pipeline（避开 pipeline 的兼容坑）
    ✅ 走：processor -> model.generate -> (CTC collapse) -> tokenizer.decode
    ✅ 支持长音频：chunk + overlap
    """
    ...
    def __init__(
        ...
        self.device = torch.device("cuda" if (force_cuda or torch.cuda.is_available()) else "cpu")

        use_fp16 = os.getenv(MED_ASR_USE_FP16_ENV, "").strip().lower() in ("1", "true", "yes", "y")
    ...
    def _chunk_waveform(self, wav_path: str) -> List[Any]:
        """
        用 torchaudio 读 wav，然后按 chunk_length_s/stride_length_s 分段。
        返回 list[numpy_1d]
        """
        import torchaudio
