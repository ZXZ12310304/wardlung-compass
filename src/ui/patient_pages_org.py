...
def render_patient_page(state: dict, ctx: dict) -> str:
    ...
    def _nav_js(page_key: str) -> str:
        js = (
            "(function(){var root=document.querySelector('gradio-app');"
            "var dom=root&&root.shadowRoot?root.shadowRoot:document;"
            f"var p='{page_key}';"
            "try{localStorage.setItem('wl_page', p);}catch(e){};"
            "window._wl_page=p;"
            "var navs=dom.querySelectorAll('.nav-item[data-page]');"
            "navs.forEach(function(n){if(n.getAttribute('data-page')===p){n.classList.add('active');}else{n.classList.remove('active');}});"
            "var sections=dom.querySelectorAll('.page-section[data-page]');"
            "sections.forEach(function(s){s.style.display=(s.getAttribute('data-page')===p)?'block':'none';});"
            "})(); return false;"
        )
    ...
    <div class="care-card" style="position:relative;">
      <div class="actions">
        <button class="icon-btn" onclick="{ctx['onclick']('do_tts')}">{icons['play']} TTS</button>
...
"""
    ...
    if step == 1:
        ...
        content = "".join([
            ...
            "<div class=\"dc-voice\" onclick=\"" + ctx["dc_onclick"]("dc_voice") + "\">Voice-to-text (coming soon)</div>",
        ])
...
def _render_care_cards_main(state: dict, ctx: dict) -> str:
    ...
    search_send_js = (
        "(function(){var root=document.querySelector('gradio-app');"
        "var dom=root&&root.shadowRoot?root.shadowRoot:document;"
        "var el=dom.querySelector('#care_search_input');"
        "var val=el?el.value:'';"
        "var input=dom.querySelector('#ui_payload textarea, #ui_payload input');"
        "if(input){input.value=JSON.stringify({q: val});"
        "input.dispatchEvent(new Event('input',{bubbles:true}));}"
        "var btn=dom.querySelector('#care_search');if(btn)btn.click();})();"
    )
...
"""
    ...
    if modal_id:
        ...
        focus = html.escape(card.get("one_liner") or "")
        do_list = "".join(f"<li>{html.escape(b)}</li>" for b in (card.get("bullets") or []))
        dont_list = "".join(f"<li>{html.escape(b)}</li>" for b in (card.get("follow_up") or []))
        help_list = "".join(f"<li>{html.escape(b)}</li>" for b in (card.get("red_flags") or []))
        focus_html = f"<div class='care-focus'>{focus}</div>" if focus else ""
        modal_html = f"""
...
<div class='care-modal' onclick="event.stopPropagation();">
  ...
  <div class='care-modal-date'>{html.escape(card.get('date',''))}</div>
  {focus_html}
...
def _render_chat_main(state: dict, ctx: dict) -> str:
    ...
    bubbles = "".join(
        f"<div class='bubble {'user' if m['role']=='user' else 'assistant'}'>{html.escape(m['text'])}</div>"
        for m in history[-8:]
    )
...
"""
    ...
    send_js = "(function(){var root=document.querySelector('gradio-app');var dom=root&&root.shadowRoot?root.shadowRoot:document;var inputEl=dom.querySelector('#chat_input');if(!inputEl) return;var msg=inputEl.value; if(!msg) return; var thinking=dom.querySelector('#chat_thinking'); if(thinking){thinking.style.display='block';} var page=(window._wl_page||(function(){try{return localStorage.getItem('wl_page')||'';}catch(e){return '';}})()); var input=dom.querySelector('#ui_payload textarea, #ui_payload input'); if(input){input.value=JSON.stringify({message: msg, current_page: page}); input.dispatchEvent(new Event('input',{bubbles:true}));} var btn=dom.querySelector('#chat_send'); if(btn) btn.click(); inputEl.value='';})();"

    mic_down_js = "(function(){var root=document.querySelector('gradio-app');var dom=root&&root.shadowRoot?root.shadowRoot:document;var btn=dom.querySelector('#chat_mic_btn');if(btn) btn.classList.add('recording');if(window._wl_rec && window._wl_rec.state==='recording') return; if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){var b=dom.querySelector('#chat_voice'); if(b) b.click(); return;} navigator.mediaDevices.getUserMedia({audio:true}).then(function(stream){window._wl_stream=stream;var rec=new MediaRecorder(stream);window._wl_rec=rec;window._wl_chunks=[];rec.ondataavailable=function(e){if(e.data&&e.data.size>0) window._wl_chunks.push(e.data);};rec.onstop=function(){var blob=new Blob(window._wl_chunks,{type:rec.mimeType||'audio/webm'});var reader=new FileReader();reader.onload=function(){var inputEl=dom.querySelector('#chat_input');var msg=inputEl?inputEl.value:'';var page=(window._wl_page||(function(){try{return localStorage.getItem('wl_page')||'';}catch(e){return '';}})()); var payload={message: msg, audio_b64: reader.result||'', current_page: page};var input=dom.querySelector('#ui_payload textarea, #ui_payload input'); if(input){input.value=JSON.stringify(payload); input.dispatchEvent(new Event('input',{bubbles:true}));} var thinking=dom.querySelector('#chat_thinking'); if(thinking){thinking.style.display='block';} var btnSend=dom.querySelector('#chat_send'); if(btnSend) btnSend.click(); if(inputEl) inputEl.value='';};reader.readAsDataURL(blob);if(window._wl_stream){window._wl_stream.getTracks().forEach(function(t){t.stop();});}};rec.start();}).catch(function(){var b=dom.querySelector('#chat_voice'); if(b) b.click();});})();"
    mic_up_js = "(function(){var root=document.querySelector('gradio-app');var dom=root&&root.shadowRoot?root.shadowRoot:document;var btn=dom.querySelector('#chat_mic_btn');if(btn) btn.classList.remove('recording');var rec=window._wl_rec; if(rec && rec.state==='recording'){rec.stop();}})();"
    image_js = "(function(){var root=document.querySelector('gradio-app');var dom=root&&root.shadowRoot?root.shadowRoot:document;var inp=dom.querySelector('#chat_image_upload input[type=file]'); if(inp) inp.click();})();"
...
def _render_inbox_main(state: dict, ctx: dict) -> str:
    ...
    search_send_js = (
        "(function(){var root=document.querySelector('gradio-app');"
        "var dom=root&&root.shadowRoot?root.shadowRoot:document;"
        "var el=dom.querySelector('#inbox_search_input');"
        "var val=el?el.value:'';"
        "var input=dom.querySelector('#ui_payload textarea, #ui_payload input');"
        "if(input){input.value=JSON.stringify({q: val});"
        "input.dispatchEvent(new Event('input',{bubbles:true}));}"
        "var btn=dom.querySelector('#inbox_search');if(btn)btn.click();})();"
    )
    search_keydown_js = f"if(event.key==='Enter'){{{search_send_js}}}"
    list_html = ""
    if not messages:
